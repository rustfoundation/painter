...
pub fn to_json<'a>(sources: &'a CrateCollection) -> Result<(), DependsError> {
    ...
    std::fs::write(
        crate_bc_dir.join("/home/jaynus/dev/cargo_bytecodes/nodes.json"),
        json::stringify_pretty(json_nodes, 4),
    );
    std::fs::write(
        crate_bc_dir.join("/home/jaynus/dev/cargo_bytecodes/edges.json"),
...
mod tests {
    ...
    #[test]
    fn test_json() {
        let sources = get_crate_sources(&Path::new("/home/jaynus/dev/cargo_sources")).unwrap();
